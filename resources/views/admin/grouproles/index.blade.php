				...
				<div class="table-responsive pull-right"  style="width: 100% !important">
					<table class="table table-striped table-hover">
				     ...
				     <tr>
				         ...
				         <td>
									<a href='{{URL::action("admin\GroupRoleController@edit",array($grouprole->id))}}'>edit</a>		
									<a href='{{URL::action("admin\GroupRoleController@show",array($grouprole->id))}}'>show</a>
									<form id="delete_grouprole{{$grouprole->id}}" action='{{URL::action("admin\GroupRoleController@destroy",array($grouprole->id))}}' method="POST">
									    <input type="hidden" name="_method" value="delete">
									    <input type="hidden" name="_token" value="{{ csrf_token() }}">
									    <a href="#" onclick="document.getElementById('delete_grouprole{{$grouprole->id}}').submit();">delete</a>
									</form>
								  </td>
				     	</tr>
					   @endforeach
